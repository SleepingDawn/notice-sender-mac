...
struct SendCommand: ParsableCommand {
    ...
    @Flag(name: .long, help: "Open and verify the exact room and composer without sending")
    var verifyOnly: Bool = false

    @Flag(name: .long, help: "Match only a normalized room title that is exactly equal")
    var exact: Bool = false

    @Flag(name: .long, help: "Fail unless exactly one matching result is exposed")
    var requireUnique: Bool = false

    @Flag(name: .long, help: "Fail instead of using the forced-typing fallback")
    var noForcedTyping: Bool = false

    @Flag(name: .long, help: "Print a KMSG_SAFE_RESULT JSON marker on success")
    var json: Bool = false
    ...
    func run() throws {
        ...
        do {
            ...
            if verifyOnly {
                try verifyMessageInput(in: resolution.window, kakao: kakao, runner: runner)
                emitSafeResult(action: "verify", roomTitle: resolution.window.title ?? "")
            ...
            emitSafeResult(action: "send", roomTitle: resolution.window.title ?? "")
            closeWindowsIfNeeded(
                resolution: resolution,
                kakao: kakao,
                resolver: chatWindowResolver,
    ...
    private func sendMessageToWindow(_ window: UIElement, kakao: KakaoTalkApp, runner: AXActionRunner) throws {
        ...
        guard let input = resolveMessageInputField(chatWindow: window, kakao: kakao, runner: runner) else {
            if noForcedTyping {
                throw KakaoTalkError.actionFailed("[FORCED_TYPING_DISABLED] Message input field was not found")
            }
    ...
    private func verifyMessageInput(in window: UIElement, kakao: KakaoTalkApp, runner: AXActionRunner) throws {
        guard resolveMessageInputField(chatWindow: window, kakao: kakao, runner: runner) != nil else {
            throw KakaoTalkError.actionFailed("[COMPOSER_NOT_FOUND] Message input field was not found")
        }
        print("✓ Exact room and message input verified for \(targetDescription)")
    }

    private func emitSafeResult(action: String, roomTitle: String) {
