...
final class ChatIdentityRegistryStore: @unchecked Sendable {
    ...
    func assignChatIDs(for discoveries: [ChatListDiscovery]) -> [String] {
        ...
        let groupedCurrent = Dictionary(grouping: discoveries.indices) { index in
            ChatTextNormalizer.normalizeTitleIdentity(discoveries[index].title)
        }
    ...
    private func updatedRecord(
        ...
        updated.normalizedName = ChatTextNormalizer.normalizeTitleIdentity(discovery.title)
        ...
        do {
            ...
            if document.schemaVersion == 1 {
                ...
                for index in document.records.indices {
                    document.records[index].normalizedName = ChatTextNormalizer.normalizeTitleIdentity(document.records[index].displayName)
                }
