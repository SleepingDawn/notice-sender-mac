...
public final class KakaoTalkApp: Sendable {
    ...
    public init(autoLaunch: Bool = true) throws {
        ...
        self.processIdentifier = runningApp.processIdentifier
    ...
    public func activate() {
        ...
        // On modern macOS activation itself brings the app forward; the former
        // activateIgnoringOtherApps option is deprecated and has no effect.
    ...
    /// Explicitly switch the main KakaoTalk window to the Chats tab.
    ///
    /// KakaoTalk exposes the friends and chats screens through the same window
    /// and both contain a table/list. Scanning the first list without pressing
    /// the `chatrooms` navigation item can therefore return the friends list.
    @discardableResult
...
public enum KakaoTalkError: Error, CustomStringConvertible, LocalizedError {
    case appNotRunning
    case launchFailed
