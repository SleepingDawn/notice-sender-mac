...
struct AXActionRunner {
    ...
    private let cancellationToken: KmsgCancellationToken?
    ...
    var isCancelled: Bool { cancellationToken?.isCancelled == true }
    ...
    func waitUntil(
        ...
        return !isCancelled && evaluateAfterTimeout ? condition() : false
    }
    ...
    private func typeText(_ text: String, perCharacterDelay: TimeInterval) {
        let source = CGEventSource(stateID: targetProcessIdentifier == nil ? .hidSystemState : .privateState)
    ...
    private func postKeyboardEvent(_ event: CGEvent) {
        if let targetProcessIdentifier {
            // Deliver to the semantically resolved KakaoTalk process. This
            // avoids depending on whichever application happens to be frontmost
            // and never uses a screen coordinate.
